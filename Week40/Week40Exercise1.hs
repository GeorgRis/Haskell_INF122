...
-- PlanetarySystem -> Integer
numberOfMoons :: PlanetarySystem -> Integer
numberOfMoons system = sum (map (toInteger . length . moons) (planets system))

-- PlanetarySystem -> [Planet]
atLeastOneMoon :: PlanetarySystem -> [Planet]
atLeastOneMoon system = filter (\p -> not (null (moons p))) (planets system)
