...
instance Ranged CellRef where
    data Dimension CellRef
       = Dimension { columns :: [Char]
                   , rows :: [Integer] }
    cellRange dim box = Set.fromList
        [ Cell c r
        | c <- columns dim, r <- rows dim
        , c >= column (upperLeft box), c <= column (lowerRight box)
        , r >= row (upperLeft box), r <= row (lowerRight box)
        ]
...
evaluate :: (Num number, Ranged cell)
         ...
         -> Maybe number
evaluate sheet expr = evaluate' Set.empty sheet expr

evaluate' :: (Num number, Ranged cell)
          => Set cell -- Akkumulator for celler som er under evaluering
          -> Sheet number cell
          -> Expression number cell
          -> Maybe number
evaluate' _ _ (Constant n) = Just n
evaluate' under_eval sheet (Ref cell) = do
    guard (cell `Set.notMember` under_eval) -- Sjekk for syklisk referanse (del d)
    expr <- Map.lookup cell (content sheet)
    evaluate' (Set.insert cell under_eval) sheet expr
evaluate' under_eval sheet (Add a b) = do
    a' <- evaluate' under_eval sheet a
    b' <- evaluate' under_eval sheet b
    return (a' + b')
evaluate' under_eval sheet (Mul a b) = do
    a' <- evaluate' under_eval sheet a
    b' <- evaluate' under_eval sheet b
    return (a' * b')
evaluate' under_eval sheet (Sub a b) = do
    a' <- evaluate' under_eval sheet a
    b' <- evaluate' under_eval sheet b
    return (a' - b')
evaluate' under_eval sheet (Sum range) = do
    let cells = cellRange (dimension sheet) range
    let exprs = sequence $ map (`Map.lookup` content sheet) (Set.toList cells)
    -- Henter ut uttrykkene for cellene i området. 'sequence' returnerer Nothing
    -- hvis en celle i området er utenfor arket eller er tom (ikke i 'content').
    expressions <- exprs
    -- Rekursivt evaluerer alle uttrykkene i området
    let evaluations = map (evaluate' under_eval sheet) expressions
    values <- sequence evaluations -- 'sequence' returnerer Nothing hvis en av evalueringene feiler
    return (sum values)
...
pBetween pOpen pClose pContent = do
    pOpen
    x <- pContent
    pClose
    return x
...
pCell = Cell <$> pColName <*> pRowNumber
...
pRef = pChar '!' *> (Ref <$> pCell)

-- | Parse a sum of cell refences like SUM(A1:C3)
pSum :: Read number => Parser (Expression number CellRef)
pSum = do
    pKeyword "SUM"
    inParenthesis $ do
        ul <- pCell
...
pRow cols = do
    r <- inBrackets pRead
    pMany pSpace
    exprs <- pSepBy1 pSpace (inBrackets pExpression)
    pMany pSpace
    pNewLine <|> pure ()
    let cells = [ (Cell c r, expr)
                | (c, expr) <- zip cols exprs ]
    return (r, cells)
...
pSheet = do
    ...
    pChar ']'
    pMany pSpace
    cols <- pSepBy1 pSpace (inBrackets pColName)
    pNewLine

    rowsContent <- pSome (pRow cols)

    let allCells = concatMap snd rowsContent
    let sheetRows = map fst rowsContent
    let dim = Dimension cols (rows sheetRows)

    return $ Sheet
        { name = name
        , dimension = dim
        , content = Map.fromList allCells
        }
    where rows l = Set.toList (Set.fromList l)
